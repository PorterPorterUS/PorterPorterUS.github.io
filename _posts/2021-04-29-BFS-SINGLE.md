...
1.关于visited，一种办法是使用hashset,然后每次check contain，但是效率太慢，对于矩阵类型的题目，可以使用boolean[][]进行存储，每次check是否当前位置为true即可



2.visited另外一种优化方式，是把走过的位置设置为##，然后再check function中加入这一个判断条件即可



3.记录最短路径的时候，res++一定放在while loop中的最后一句，然后如果碰到目的地直接返回。不需要使用PQ存储所有抵达路径的长度，然后排序，这样太低效率
