...
所以，我们可以使用sequence number 或者 timestamp，他们一般来说都是total order的, 每个操作都有唯一的ID，任意两个操作都可以被对比。
如果A happens before B，那么A sequence number一定在B之前。如果是并发的操作，无法保证sequence number是否递增以及有顺序。
在single-leader replication中, leader可以对每个operation都加1操作，维护递增性
